...
enum VendingMachineError: Error {
    case invalidInput
    case insufficientFunds(moneyNeeded: Int)    //돈이 모자란 경우
    case outOfStock
}
...
class VendingMachine {
    ...
    func vend(numberOfItems numberOfItemsToVend: Int) throws -> String {
        ...
        guard numberOfItemsToVend > 0 else {
            throw VendingMachineError.invalidInput
            // return type이 String인데, String을 리턴하지 않고 throw하고 그냥 끝냄
        }
...
result = try! machine.vend(numberOfItems: 1)    // ->옵셔널이 아닌 값을 리턴받음
result // 1개 제공함
